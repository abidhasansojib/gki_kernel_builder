...
* **Action Launcher Public Storage Transition:** Updated `action.sh` to avoid restricted `/data/adb` paths for browser inspection and added operational guidance directing users to the root manager WebUI button.
...
* **NetHunter skip_mount Decoupling & Vendor Firmware Path Preservation:** Injected `skip_mount` into the NetHunter module package to permanently eliminate `/system` partition hijacking and VFS collisions with NoMount. Replaced unconditional `/sys/module/firmware_class/parameters/path` overwrites with safe prepends (`$MODDIR/firmware,$CURR_FW`), ensuring internal device Wi-Fi/modem/DSP vendor firmware paths (`/vendor/firmware`) remain fully intact while seamlessly loading external USB Wi-Fi dongle firmwares.
* **Expanded USB ConfigFS & NetHunter Subsystem Gadget Configs:** Added extended USB Gadget ConfigFS functions and subsystem configuration options (`CONFIG_USB_CONFIGFS_UEVENT`, `CONFIG_USB_CONFIGFS_F_AC`, `CONFIG_USB_CONFIGFS_F_AUDIO_SRC`, `CONFIG_USB_CONFIGFS_F_CCID`, `CONFIG_USB_CONFIGFS_F_CDEV`, `CONFIG_USB_CONFIGFS_F_DIAG`, `CONFIG_USB_CONFIGFS_F_GSI`, `CONFIG_USB_CONFIGFS_F_LB_SS`, `CONFIG_USB_CONFIGFS_F_PRINTER`, `CONFIG_USB_CONFIGFS_F_QDSS`, `CONFIG_USB_CONFIGFS_F_UAC1`, `CONFIG_USB_CONFIGFS_F_UAC1_LEGACY`, `CONFIG_IIO_CONFIGFS`, `CONFIG_PCI_ENDPOINT_CONFIGFS`) into `nethunter/action.yml` for comprehensive USB gadget capabilities.
* **Robust WebUI BadUSB ConfigFS & UDC Binding Sequence:** Updated NetHunter WebUI (`webroot/index.html`) USB HID configuration to adopt robust ConfigFS discovery, UDC conflict unbinding/binding loop, standard 63-byte keyboard descriptor, 52-byte 4-report mouse descriptor (with wheel support), dual test injection buttons (keystroke 'a' & mouse left-click), and clean stock Android restoration.
* **Dynamic BadUSB Device Node Auto-Healing (`/dev/hidg0` & `/dev/hidg1`):** Discovered that Android stock init pre-allocates minor 0 at `/config/usb_gadget/g1/functions/hid.gs0`, causing custom gadget `f_hid` functions to receive minor offsets (`485:1` for Keyboard and `485:2` for Mouse), while Android `ueventd` fails to auto-create character nodes for dynamic majors (> 255). Implemented dynamic `mknod -m 666` auto-healing in `setUsbMode('hid')` and `checkUsbStatus()` to guarantee both `/dev/hidg0` (Keyboard) and `/dev/hidg1` (Mouse) are created and active. Added proper ConfigFS teardown (unbinding UDC first, using `rmdir` for functions, absolute symlinks, and device node cleanup upon stock reset).
* **NetHunter WebUI 4-Tab Streamlining & Mobile UX:** Streamlined navigation into 4 dedicated tabs (Network, USB, Drivers, Memory) by removing the redundant Terminal tab in favor of the integrated Process & Activity Log.
* **Process & Activity Log Modernization:** Enhanced the mobile activity drawer with Material 3 pill counters, command execution tracking, and one-tap Copy/Clear controls; removed the intrusive autoscroll toggle button in favor of native mobile touch scrolling.
* **Sequential Driver Loading Queue & Variable Escaping:** Resolved WebUI browser lockups during "Load All Modules" by replacing parallel shell execution with a sequential asynchronous queue providing real-time UI progress feedback. Fixed a JavaScript `ReferenceError: base is not defined` caused by unescaped `${base}` template literal parsing in `autoLoadModule()` and `loadAllAvailableModules()`.
* **MGLRU & Kernel Memory Tuning Alignment:** Documented MGLRU bitmask architectures (`0x0001` core, `0x0002` leaf, `0x0004` non-leaf = `0x0007` full acceleration), verified in-tree kernel patches (`performance/action.yml`), module boot services (`service.sh`), and WebUI one-tap locking with architecture clipping detection.
* **mac80211 Stock GKI Detection & Dynamic Module Cycling:** Resolved module loading failures (`err -2: Unknown symbol __ieee80211_*_led_*` and CRC divergence) for `ath9k_htc`, `carl9170`, `rtl8187`, and `rt2800usb` (via `rt2x00lib`). Discovered that stock Android GKI early init mounts `/system_dlkm` and loads a stripped `mac80211.ko` lacking `CONFIG_MAC80211_LEDS=y`. Because `/sys/module/mac80211` was already present, previous loader scripts bypassed loading the NetHunter `mac80211.ko`. Implemented `ensure_mac80211()` across `index.html`, `post-fs-data.sh`, and `service.sh` to audit `/proc/kallsyms` for `__ieee80211_create_tpt_led_trigger`, dynamically unload dependent holders across 3 passes, swap the stock module for the full NetHunter `mac80211.ko`, and restore dependent drivers seamlessly.
* **USB Gadget ConfigFS Multi-Gadget Support (`patch_usb_gadget.py`):** Fixed `mkdir /config/usb_gadget/<custom_gadget>` failing with false `Out of memory` (`ENOMEM`) in Android GKI kernels. Discovered via live dynamic `ftrace` + `kprobe_events` and dmesg audit that `android_device_create()` collided on duplicate sysfs entries `/sys/class/android_usb/android1` (`-EEXIST`), which `configfs.c` masked as `-ENOMEM`. Created `.github/scripts/patch_usb_gadget.py` and integrated into `kernel-fixes/action.yml` to allow non-system gadgets (e.g., `gadgetfs_*`, Stryker) to bypass `android_device_create()` while preserving full Android OS uevent delivery for the primary `g1`/`g0` gadget, with NULL-safe `android_device_destroy()` guard in `gadgets_drop()`.
* **NetHunter WebUI USB & HID Tab Decoupling:** Removed the USB & HID tab from the WebUI (`webroot/index.html`) per user request to delegate USB HID and BadUSB attack orchestration directly to dedicated Android apps (e.g., Kali NetHunter app, Tap Ducky, Stryker). Streamlined the WebUI navigation bar to 4 balanced primary modules (Network, Drivers, Memory, Logs) and synced changes across repository and local internal storage mirrors.
* **Repository Cleanup & Documentation Audit:** Removed `.vscode/` editor directory and ignored it in `.gitignore`; rewrote project `LICENSE` specifically for `gki_kernel_builder` under GPL-3.0-or-later with full unabridged GPLv3 text; added automated developer/testing warning notices to GitHub Pre-Release builds in `build.yml`; audited BadUSB device node mappings (`/dev/hidg1` keyboard, `/dev/hidg2` mouse); and verified the supported wireless Wi-Fi adapter table in `README.md` and `docs/ROOT_VARIANTS.md`.
* **Release, Tag & Workflow History Reset:** Purged all historical releases and Git tags from the repository, cleaned out past GitHub Actions run history, and renamed `.github/workflows/build.yml` to `.github/workflows/build-kernel.yml` to reset GitHub's internal workflow run counter so the next dispatched build begins cleanly at Run #1.
