...
check_config() {
    ...
    case "$fallback_type" in
        ...
        module_file)
            local mod_found=0
            for dir in /data/adb/modules/nethunter_wireless_modules/lkm \
                       /data/adb/modules/nethunter_wireless_modules/system/lib/modules \
                       /data/adb/modules/nethunter_wireless/lkm \
...
check_firmware() {
    ...
    for prefix in /vendor/firmware /vendor/etc/firmware /system/etc/firmware \
                  /data/adb/modules/nethunter_wireless_modules/system/etc/firmware \
                  /data/adb/modules/nethunter_wireless_modules/vendor/firmware \
...
if [ "$FAILED_COUNT" -eq 0 ]; then
    ...
    echo -e "${YELLOW}${BOLD}⚠ NOTE: Flash Nethunter-Wireless-Modules.zip in KernelSU-Next / SukiSU-Ultra / ReSukiSU to load modular drivers.${NC}\n"
fi
