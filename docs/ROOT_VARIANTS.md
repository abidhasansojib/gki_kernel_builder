...
The build system supports two distinct root flavors selectable via the `root_flavor` workflow input. Each flavor is isolated and cleanly integrated during build time:
...
* **Flashable Wireless & Driver Module**:
  All compiled `.ko` driver modules, official Linux firmware blobs, and HID permission daemons are packaged into a standalone KernelSU-Next / ReSukiSU module (`Nethunter-Wireless-Modules.zip`) with an automated boot-time `service.sh` driver loader.
...
2. **`Nethunter-Wireless-Modules.zip`**: Flashable KernelSU-Next / ReSukiSU module for external USB WiFi dongles and firmware.
